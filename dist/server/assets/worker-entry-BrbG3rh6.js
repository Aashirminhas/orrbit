...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("./_tanstack-start-manifest_v-DqkLiqUs.js");
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import("./router-BidXzqNf.js").then((n2) => n2.r),
  ...
  const startRequestResolver = async (request, requestOpts) => {
    ...
    try {
      ...
      const executeRouter = async (serverContext, matchedRoutes) => {
        ...
        const onEarlyHints = false ? void 0 : requestOpts?.onEarlyHints;
        const earlyHintsSentLinks = onEarlyHints ? /* @__PURE__ */ new Set() : void 0;
        const earlyHintsSentHints = onEarlyHints ? new Array() : void 0;
        if (onEarlyHints && earlyHintsSentLinks && earlyHintsSentHints && matchedRoutes?.length) sendEarlyHintsEvent({
          phase: "static",
        ...
        if (onEarlyHints && earlyHintsSentLinks && earlyHintsSentHints) sendEarlyHintsEvent({
          phase: "dynamic",
